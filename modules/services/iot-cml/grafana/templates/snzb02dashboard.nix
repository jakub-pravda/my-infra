...
let
  ...
  snzbDevicesTemplates = pkgs.lib.flatten (
    map (
      hubConfig:
      (map (
        device:
        dashboardTemplate "${device.location}/${device.name}" "${hubConfig.id}/${device.location}/${device.name}"
      ) (snzbFilter hubConfig.devices))
  ...
  dashboardDir =
    let
      linkTemplate = map (
        templatePath: "ln -s ${templatePath} $out/${builtins.baseNameOf templatePath}.json"
      ) snzbDevicesTemplates;
    in
    pkgs.runCommand "snzb-dashboards" { preferLocalBuild = true; } ''
      mkdir -p $out
